...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900">Ajouter du personnel</h1>
    </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-50">
            <div class="p-6 grid grid-cols-2 gap-4">
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Prénom *</label>
                    <input type="text" name="first_name" value="{{ old('first_name') }}" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-300 outline-none"></div>

                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Nom *</label>
                    <input type="text" name="last_name" value="{{ old('last_name') }}" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-300 outline-none"></div>
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Catégorie *</label>
                    <select name="category" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none bg-white">
                        <option value="administratif" {{ old('category','administratif') === 'administratif' ? 'selected':'' }}>Administratif</option>
                        <option value="appoint"       {{ old('category') === 'appoint' ? 'selected':'' }}>Appoint</option>
                    </select></div>

                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Poste *</label>
                    <input type="text" name="position" value="{{ old('position') }}" required placeholder="Ex: Secrétaire, Gardien..." class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-300 outline-none"></div>
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Salaire de base (FCFA) *</label>
                    <input type="number" name="base_salary" value="{{ old('base_salary') }}" required min="0" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-300 outline-none"></div>
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Type contrat</label>
            ...
            <div class="px-6 py-4 flex justify-between bg-gray-50/50 rounded-b-2xl">
                <a href="{{ route('staff.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Annuler</a>
